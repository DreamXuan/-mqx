...
<body>
			...
			<ul class="nav navbar-nav navbar-right">
				<c:choose>
					<c:when test="${user.userid!=null}">
						<li>
							<a>
								<c:out value="${user.usertruename}" />您好
							</a>
						</li>
						</li>
	...
	<script>
		...
		function getScore(){
			//var spid = n
			$.ajax({
				url: '/getscore?userid='+'${user.userid}'+'&spid='+ now.getTime(),
				type:'post',
				dataType:'json',
				contentType: false,
				processData: false,
				success: function(res) {
					...
					layer.open({
						title:'得分',
						content: "您本次得分" +res.data * 2 + "分!",
						icon:1,
						end:function(){
							location.href = '/errorpaperlist';
						}
					})
				},
				error: function(res) {
